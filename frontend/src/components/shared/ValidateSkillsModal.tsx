...
export function ValidateSkillsModal({ student, onClose, onSaved }: ValidateSkillsModalProps) {
  ...
  const titleId = 'validate-skills-title'
  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
      ...
      <div
        ...
        tabIndex={-1}
        className="relative w-full max-w-2xl max-h-[85vh] flex flex-col rounded-2xl bg-surface shadow-2xl animate-slide-up overflow-hidden"
      >
        {/* Header */}
        <div className="flex items-center gap-4 px-6 py-5 border-b border-outline-variant/15">
          ...
          <div className="flex-1 min-w-0">
            <h2 id={titleId} className="font-headline font-bold text-on-surface text-lg truncate">{fullName}</h2>
        ...
        <div className="overflow-y-auto flex-1 p-6">
          {student.skills.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-16 text-center">
              <span className="material-symbols-outlined text-[48px] text-outline" aria-hidden="true">psychology</span>
              <p className="mt-3 text-sm font-semibold text-on-surface-variant">Este estudiante aún no declaró habilidades.</p>
            </div>
          ) : (
                        ...
                        >
                          <div className="flex items-start gap-3 flex-wrap">
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              {action && (
                                <>
                                  <label htmlFor={`skill-note-${skill.id}`} className="sr-only">
                                    Nota para {skill.name} (opcional)
                                  </label>
                                  <input
                                    ...
                                    className="mt-2 w-full bg-transparent text-xs outline-none border-b border-outline-variant pb-1 focus:border-primary focus:ring-2 focus:ring-primary/30 transition-colors placeholder:text-outline"
                                  />
                                </>
                              )}
                            </div>
                            ...
                            <div className="flex items-center gap-2 shrink-0">
                              <button
                                ...
                                aria-pressed={action === 'VALIDADA'}
                                aria-label={`Validar ${skill.name}`}
          ...
          {error && <p role="alert" className="mt-4 text-xs text-error font-semibold">{error}</p>}
        </div>
